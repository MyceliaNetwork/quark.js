...
function createCheckout({ domain, provider, x, }) {
    return function checkout(b) {
        ...
        console.log("🚀 ~ file: checkout.ts:22 ~ checkout ~ b", b);
        x.basket = b;
        console.log("🚀 ~ file: checkout.ts:23 ~ checkout ~ basket", x.basket);
        const queryString = JSON.stringify({
            origin: window.origin,
            provider: provider,
        });
        x.windowObject = window.open(`${domain}/checkout?data=${btoa(queryString)}`, "_blank");
        console.log("🚀 ~ file: checkout.ts:28 ~ checkout ~ windowObject", x.windowObject);
        return x.basket;
    };
}
...
function initialize(config) {
    ...
    window.addEventListener("message", function (event) {
        ...
        if (event.data.type === "checkoutLoaded") {
            ...
            console.log("🚀 ~ file: initialize.ts:23 ~ initialize ~ message", message);
